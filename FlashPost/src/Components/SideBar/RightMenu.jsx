...
function RightMenu() {
    ...
    const fetchTrends = async () => {
      try {
        ...
        const res = await axios.get(
          `https://localhost/3000/api/blogs/trending`
        );
  ...
  const fetchAuthorImage = async (author) => {
    try {
      const res = await axios.get(
        `http://localhost:3000/api/users/userProfile/${author}`
      );
                ...
                <div key={trend.id}>
                  <div className="flex gap-[.4em] mx-4 mt-2">
                    ...
                    <div className="ml-2">
                      ...
                      <div className="mt-1">
                        <h1 className="text-sm text-slate-500 font-normal">
                          @{trend?.Author}
                        </h1>
                      </div>
                      <div className="flex gap-3 mb-3">
                        ...
                        <button className="bg-[#f5f5f5] text-[#0a0a0a] text-xs font-medium px-2 py-1 rounded-md mt-2">
                          <TfiCommentAlt className="inline-block mr-1" />
                          87
                        </button>
                      </div>
                    </div>
